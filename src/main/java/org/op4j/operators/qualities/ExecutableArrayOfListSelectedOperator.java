 ...
 * array of list target objects on which a selection ("if") has already been done.
 * </p>
 * 
 * @since 1.0
 * 
...
public interface ExecutableArrayOfListSelectedOperator<T> {
     ...
     * @return an operator on the results of function execution
     */
    public ExecutableArrayOfListSelectedOperator<T> execIfNotNullAsArrayOfList(final IFunction<? extends List<? extends T>[],? super List<T>[]> function);
     ...
     */
    public ExecutableArrayOfListSelectedOperator<T> mapMap(final IFunction<? extends T,? super T> function);

 ...
 *
 */
public final class Level0ArrayOfMapOperatorImpl<K,V> extends AbstractOperatorImpl
    ...
    public Level0ArrayOfMapOperator<K, V> add(final Map<K, V> newElement) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.Add<Map<K, V>>(NormalisationUtils.normaliseMap(newElement))));
    }

    public Level0ArrayOfMapOperator<K, V> addAll(final Map<K, V>... newElements) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.Add<Map<K, V>>(NormalisationUtils.normaliseMaps(newElements))));
    }


    @SuppressWarnings("unchecked")
    public Level0ArrayOfMapOperator<K, V> insert(final int position, final Map<K, V> newElement) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.Insert<Map<K, V>>(position, NormalisationUtils.normaliseMap(newElement))));
    }

    public Level0ArrayOfMapOperator<K, V> insertAll(final int position, final Map<K, V>... newElements) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.Insert<Map<K, V>>(position, NormalisationUtils.normaliseMaps(newElements))));
    }


    public Level0ArrayOfMapOperator<K, V> addAll(final Collection<Map<K, V>> collection) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.AddAll<Map<K, V>>(NormalisationUtils.normaliseMaps(collection))));
    }
    ...
    public Level1ArrayOfMapElementsOperator<K, V> forEach() {
        return new Level1ArrayOfMapElementsOperatorImpl<K, V>(getTarget().iterate(Structure.ARRAY));
    }


    public Level0ArrayOfMapOperator<K, V> removeAllIndexes(final int... indexes) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllIndexes<Map<K, V>>(indexes)));
    }
    ...
    public Level0ArrayOfMapOperator<K, V> removeAllTrue(final IFunction<Boolean, ? super Map<K, V>> eval) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllTrue<Map<K, V>>(eval)));
    }

    public Level0ArrayOfMapOperator<K, V> removeAllFalse(final IFunction<Boolean, ? super Map<K, V>> eval) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllFalse<Map<K, V>>(eval)));
    }

    public Level0ArrayOfMapOperator<K, V> removeAllNullOrFalse(final IFunction<Boolean, ? super Map<K, V>> eval) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllNullOrFalse<Map<K, V>>(eval)));
    }

    public Level0ArrayOfMapOperator<K, V> removeAllNotNullAndFalse(final IFunction<Boolean, ? super Map<K, V>> eval) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllNotNullAndFalse<Map<K, V>>(eval)));
    }


    public Level0ArrayOfMapOperator<K, V> removeAllNullOrTrue(final IFunction<Boolean, ? super Map<K, V>> eval) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllNullOrTrue<Map<K, V>>(eval)));
    }


    public Level0ArrayOfMapOperator<K, V> removeAllNotNullAndTrue(final IFunction<Boolean, ? super Map<K, V>> eval) {
    ...
    public Level0ArrayOfMapOperator<K, V> removeAllIndexesNot(final int... indexes) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().execute(new ArrayFuncs.RemoveAllIndexesNot<Map<K, V>>(indexes)));
    }
	...
	public Level0ListOfMapOperator<K, V> toListOfMap() {
	    return new Level0ListOfMapOperatorImpl<K, V>(getTarget().execute(new ToList.FromArray<Map<K,V>>()));
    }


	public <K1> Level0MapOfMapOperator<K1, K, V> toMapOfMap(final IFunction<K1, ? super Map<K, V>> keyEval) {
        return new Level0MapOfMapOperatorImpl<K1, K, V>(getTarget().execute(new ToMap.FromArrayByKeyEval<K1,Map<K,V>>(keyEval)));
    }


	public <K1, K2, V2> Level0MapOfMapOperator<K1, K2, V2> toMapOfMap(final IMapBuilder<K1, Map<K2, V2>, ? super Map<K, V>> mapBuild) {
        return new Level0MapOfMapOperatorImpl<K1, K2, V2>(getTarget().execute(new ToMap.FromArrayByMapBuilder<K1,Map<K2,V2>,Map<K,V>>(mapBuild)));
    }


	public Level0SetOfMapOperator<K, V> toSetOfMap() {
        return new Level0SetOfMapOperatorImpl<K, V>(getTarget().execute(new ToSet.FromArray<Map<K,V>>()));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifIndex(final int... indexes) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectIndex(indexes));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifIndexNot(final int... indexes) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectIndexNot(indexes));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifTrue(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectMatching(eval));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifFalse(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
    ...
    public Level0ArrayOfMapSelectedOperator<K, V> ifNotNullAndTrue(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectNotNullAndMatching(eval));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifNotNullAndFalse(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
    ...
    public Level0ArrayOfMapSelectedOperator<K, V> ifNullOrTrue(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectNullOrMatching(eval));
    }


    public Level0ArrayOfMapSelectedOperator<K, V> ifNullOrFalse(final IFunction<Boolean, ? super Map<K, V>[]> eval) {
        return new Level0ArrayOfMapSelectedOperatorImpl<K, V>(getTarget().selectNullOrNotMatching(eval));
    }


    public <X, Y> Level0ArrayOfMapOperator<X, Y> execIfNotNullAsArrayOfMap(final IFunction<? extends Map<X, Y>[], ? super Map<K, V>[]> function) {
        return new Level0ArrayOfMapOperatorImpl<X, Y>(getTarget().executeIfNotNull(function, Normalisation.ARRAY_OF_MAP));
    }


    public <X, Y> Level0ArrayOfMapOperator<X, Y> execAsArrayOfMap(final IFunction<? extends Map<X, Y>[], ? super Map<K, V>[]> function) {
        return new Level0ArrayOfMapOperatorImpl<X, Y>(getTarget().execute(function, Normalisation.ARRAY_OF_MAP));
    }
    ...
    public Level0ArrayOfMapOperator<K, V> replaceWith(final Map<K, V>[] replacement) {
        return new Level0ArrayOfMapOperatorImpl<K, V>(getTarget().replaceWith(replacement, Normalisation.ARRAY_OF_MAP));
    }

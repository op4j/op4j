 ...
 *
 */
public final class Level1SetOfSetElementsOperatorImpl<T> extends AbstractOperatorImpl
    ...
    public Level1SetOfSetElementsOperator<T> replaceWith(final Set<T> replacement) {
        return new Level1SetOfSetElementsOperatorImpl<T>(getTarget().replaceWith(replacement, Normalisation.SET));
    }
    ...
    public <X> Level1SetOfSetElementsOperator<X> map(final IFunction<X, ? super T> function) {
        return new Level1SetOfSetElementsOperatorImpl<X>(getTarget().map(Structure.SET, function, null));
    }

...
public class Level1SetOfListElementsOperator<T> extends Operator
    ...
    public ILevel1SetOfListElementsOperator<T> add(final T... newElements) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.add(newElements)));
    }


    public ILevel1SetOfListElementsOperator<T> insert(final int position, final T... newElements) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.insert(position, newElements)));
    }
    ...
    public ILevel1SetOfListElementsOperator<T> removePositions(final int... positions) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removePositions(positions)));
    }


    public ILevel1SetOfListElementsOperator<T> removeValues(final T... values) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeValues(values)));
    }


    public ILevel1SetOfListElementsOperator<T> removeMatching(final String expression, final Object... optionalExpParams) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeMatching(expression, optionalExpParams)));
    }


    public ILevel1SetOfListElementsOperator<T> removeSelected(final ISelect<T> selector) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeSelected(selector)));
    }


    public ILevel1SetOfListElementsOperator<T> removeAllExceptPositions(final int... positions) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeAllExceptPositions(positions)));
    }



    public ILevel1SetOfListElementsOperator<T> removeNotNullsMatching(final String expression, final Object... optionalExpParams) {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeNotNullsMatching(expression, optionalExpParams)));
    }


    public ILevel1SetOfListElementsOperator<T> removeNulls() {
        return new Level1SetOfListElementsOperator<T>(getTarget().execute(ListFunc.removeNulls()));
    }

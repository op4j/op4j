...
public class Level2ListOfMapElementsEntriesSelectedOperatorImpl<K,V> extends AbstractOperatorImpl implements Level2ListOfMapElementsEntriesSelectedOperator<K,V> {
    ...
    public Level2ListOfMapElementsEntriesSelectedOperator<K,V> replaceBy(final Entry<K,V> replacement) {
        return new Level2ListOfMapElementsEntriesSelectedOperatorImpl<K,V>(getTarget().replaceBy(replacement));
    }

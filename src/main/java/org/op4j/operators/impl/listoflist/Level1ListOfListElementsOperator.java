...
public class Level1ListOfListElementsOperator<T> extends Operator
    ...
    public ILevel1ListOfListElementsOperator<T> add(final T... newElements) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.add(newElements)));
    }


    public ILevel1ListOfListElementsOperator<T> insert(final int position, final T... newElements) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.insert(position, newElements)));
    }


    public ILevel1ListOfListElementsOperator<T> addAll(final Collection<T> collection) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.addAll(collection)));
    }


    public ILevel1ListOfListElementsOperator<T> distinct() {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.distinct()));
    }
    ...
    public ILevel1ListOfListElementsOperator<T> removePositions(final int... positions) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removePositions(positions)));
    }


    public ILevel1ListOfListElementsOperator<T> removeValues(final T... values) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeValues(values)));
    }


    public ILevel1ListOfListElementsOperator<T> removeMatching(final String expression, final Object... optionalExpParams) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeMatching(expression, optionalExpParams)));
    }


    public ILevel1ListOfListElementsOperator<T> removeSelected(final ISelect<T> selector) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeSelected(selector)));
    }


    public ILevel1ListOfListElementsOperator<T> removeAllExceptPositions(final int... positions) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeAllExceptPositions(positions)));
    }



    public ILevel1ListOfListElementsOperator<T> removeNotNullsMatching(final String expression, final Object... optionalExpParams) {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeNotNullsMatching(expression, optionalExpParams)));
    }


    public ILevel1ListOfListElementsOperator<T> removeNulls() {
        return new Level1ListOfListElementsOperator<T>(getTarget().execute(ListFuncOLD.removeNulls()));
    }

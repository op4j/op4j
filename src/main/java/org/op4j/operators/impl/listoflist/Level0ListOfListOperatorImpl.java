...
public class Level0ListOfListOperatorImpl<T> extends AbstractOperatorImpl
    ...
    public Level0ListOfListOperator<T> add(final List<T>... newElements) {
        return new Level0ListOfListOperatorImpl<T>(getTarget().execute(new ListFuncs.Add<List<T>>(newElements)));
    }


    public Level0ListOfListOperator<T> insert(final int position, final List<T>... newElements) {
        return new Level0ListOfListOperatorImpl<T>(getTarget().execute(new ListFuncs.Insert<List<T>>(position, newElements)));
    }


    public Level0ListOfListOperator<T> addAll(final Collection<List<T>> collection) {
        return new Level0ListOfListOperatorImpl<T>(getTarget().execute(new ListFuncs.AddAll<List<T>>(collection)));
    }

...
public class Level0MapOperator<K,V> extends Operator
    ...
    public ILevel0ListOperator<V> extractValues() {
        return new Level0ListOperator<V>(getTarget().execute(new MapFunc.ExtractValues<K, V>()));
    }
    ...
    public ILevel1MapEntriesOperator<K, V> forEachEntryMatching(final IEval<Boolean, ? super Entry<K, V>> eval) {
        return new Level1MapEntriesOperator<K, V>(getTarget().iterate(eval));
    }


    public ILevel1MapEntriesOperator<K, V> forEachEntrySelected(final ISelect<Entry<K, V>> selector) {
    ...
    public ILevel0MapOperator<K, V> removeMatching(final IEval<Boolean, ? super Entry<K, V>> eval) {
        return new Level0MapOperator<K, V>(getTarget().execute(new MapFunc.RemoveMatching<K, V>(eval)));
    }

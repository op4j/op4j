...
public class Level0MapOfListOperator<K,V> extends Operator
    ...
    public ILevel0MapOfListOperator<K, V> removeAllWithKeys(final K... keys) {
        return new Level0MapOfListOperator<K, V>(getTarget().execute(MapFunc.removeAllWithKeys(keys)));
    }


    public ILevel0MapOfListOperator<K, V> removeAll(final String expression, final Object... optionalExpParams) {
        return new Level0MapOfListOperator<K, V>(getTarget().execute(MapFunc.removeAll(expression, optionalExpParams)));
    }


    public ILevel0MapOfListOperator<K, V> removeAll(final ISelect<Entry<K, List<V>>> selector) {
        return new Level0MapOfListOperator<K, V>(getTarget().execute(MapFunc.removeAll(selector)));
    }


    public ILevel0MapOfListOperator<K, V> removeAllWithKeysNot(final K... keys) {
        return new Level0MapOfListOperator<K, V>(getTarget().execute(MapFunc.removeAllWithKeysNot(keys)));
    }

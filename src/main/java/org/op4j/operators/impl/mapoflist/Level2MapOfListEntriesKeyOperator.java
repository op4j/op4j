...
public class Level2MapOfListEntriesKeyOperator<K,V> extends Operator
    ...
    public <X> ILevel2MapOfListEntriesKeyOperator<X, V> convert(final IConverter<X, ? super K> converter) {
        return new Level2MapOfListEntriesKeyOperator<X, V>(getTarget().execute(converter));
    }


    public <X> ILevel2MapOfListEntriesKeyOperator<X, V> eval(final IEvaluator<X, ? super K> eval) {
        return new Level2MapOfListEntriesKeyOperator<X, V>(getTarget().execute(eval));
    }


    public <X> ILevel2MapOfListEntriesKeyOperator<X, V> exec(final IFunction<X, ? super K> function) {
        return new Level2MapOfListEntriesKeyOperator<X, V>(getTarget().execute(function));
    }

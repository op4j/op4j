 ...
 *
 */
public final class Level0SetOfMapOperatorImpl<K,V> extends AbstractOperatorImpl
    ...
    public <X, Y> Level0SetOfMapOperator<X, Y> execIfNotNullAsSetOfMap(final IFunction<? extends Set<? extends Map<X, Y>>, ? super Set<Map<K, V>>> function) {
        return new Level0SetOfMapOperatorImpl<X, Y>(getTarget().executeIfNotNull(function, Normalisation.SET_OF_MAP));
    }
    ...
    public Level0SetOfMapOperator<K, V> replaceWith(final Set<Map<K, V>> replacement) {
        return new Level0SetOfMapOperatorImpl<K, V>(getTarget().replaceWith(replacement, Normalisation.SET_OF_MAP));
    }

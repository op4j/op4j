...
import org.op4j.op.intf.qualities.IOperator;
...
public class Level0MapOperator<K,V> extends Operator
    ...
    public ILevel1MapEntriesOperator<K, V> forEachEntryNot(final String expression, final Object... optionalExpParams) {
        return new Level1MapEntriesOperator<K, V>(getTarget().iterateNot(expression, optionalExpParams));
    }


    public ILevel1MapEntriesOperator<K, V> forEachEntryNot(final ISelect<Entry<K, V>> selector) {
        return new Level1MapEntriesOperator<K, V>(getTarget().iterateNot(selector));
    }


    public <X, Y> ILevel0MapOperator<X, Y> of(IOf<X> ofX, IOf<Y> ofY) {
        // TODO Auto-generated method stub
        return null;
    }

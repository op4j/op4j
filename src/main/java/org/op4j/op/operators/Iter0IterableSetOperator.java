...
import org.op4j.op.interfaces.IterableSetOperator;
import org.op4j.op.interfaces.MapBuilder;
import org.op4j.operation.SetTarget;
 ...
 *
 */
public final class Iter0IterableSetOperator<T> 
        extends Iter0AbstractOperator<Set<T>>
        implements IterableSetOperator<T> {
    ...
    /*
     *  Targets are created for each object computing intended type as Set<componentType>.
     *  
     ...
     *  the user intended for the object Set is the one we could compute from
     *  its values (Integer extends Number and Integer[] extends Number[], but 
     *  Set<Integer> does not extend Set<Number>).  
     *  
     */
    @SuppressWarnings("unchecked")
    private static SetTarget<?> prepareTargets(
            final Type componentType, final Set<?> object) {
        
        return new SetTarget(componentType, object);
        
    }
    
    
    Iter0IterableSetOperator(final Class<T> operatorComponentClass, final Set<?> object) {
        this((operatorComponentClass != null? Types.getRawTypeForClass(operatorComponentClass) : null), object);
    }
    
    
    @SuppressWarnings("unchecked")
    Iter0IterableSetOperator(final Type operatorComponentType, final Set<?> object) {
        
        super((SetTarget) prepareTargets(operatorComponentType, object));
    ...
    @SuppressWarnings("unchecked")
    Iter0IterableSetOperator(final Type operatorComponentType, final Type componentType, final Set<?> object) {
        
        super((SetTarget) prepareTargets(componentType, object));
        this.operatorComponentType = operatorComponentType;
        /*
         * Check that the intended type is always compatible with the operator type.
         */
    ...
    private static void checkOperatorType(final Type operatorComponentType, final Type intendedType) {
        final Type operatorType = 
            TypeAggregationUtils.createSetOfType(operatorComponentType);
    ...
    @Override
    @SuppressWarnings("unchecked")
    final List<SetTarget<T>> getTargets() {
        return (List<SetTarget<T>>) getAbstractTargets();
    }
    ...
    final Iter0IterableSetOperator<?> unsafeRaw() {
        return new Iter0IterableSetOperator<Object>(
                (Type) null, getTargetObjects().get(0));
    }
    
    
    public final Iter0IterableSetOperator<?> raw() {
        return unsafeRaw();
    }
    
    
    final <X> Iter0IterableSetOperator<X> unsafeEachExecUniq(
            final Class<X> resultClass, final String operationName, final List<Object> parameters) {
        return unsafeEach().unsafeExecUniq(resultClass, operationName, parameters).unsafeUneachSet();
    }
    
    
    public final Iter0IterableSetOperator<?> eachExecUniq(
            final String operationName, final Object... parameters) {
        Validate.notNull(operationName, "Operation name cannot be null");
        return unsafeEachExecUniq(null, operationName, OperatorUtils.safeOperationParameters(parameters));
    }
    
    
    public final <X> Iter0IterableSetOperator<X> eachExecUniq(
        ...
        Validate.notNull(operationName, "Operation name cannot be null");
        return unsafeEachExecUniq(resultClass, operationName, OperatorUtils.safeOperationParameters(parameters));
    }
    
    
    final Iter1GenericUniqTargetOperator<T> unsafeEach() {
        final Set<T> targetObject = getTargetObjects().get(0);
    ...
    @SuppressWarnings("unchecked")
    final Iter0GenericUniqTargetOperator<Set<T>> unsafeGeneric() {
        final Target<? extends Set<T>> target = getTargets().get(0);
        return new Iter0GenericUniqTargetOperator(
                TypeAggregationUtils.createSetOfType(getOperatorComponentType()),
                target.getIntendedType(),
                target.getObject());
    }

    
    public final Iter0GenericUniqTargetOperator<Set<T>> generic() {
    ...
    final Iter0GenericMultiTargetOperator<T> unsafeFlat() {
        return new Iter0GenericMultiTargetOperator<T>(
                getOperatorComponentType(), new ArrayList<T>(getTargetObjects().get(0)));
    }
    ...
    final <X> Iter0IterableSetOperator<X> unsafeAsSet(final Class<X> ofClass) {
        ...
        return unsafeGeneric().unsafeAsSet(ofClass);
    }
    ...
    final Iter0IterableSetOperator<T> unsafeSort() {
        return unsafeSort(null);
    }


    public final Iter0IterableSetOperator<T> sort() {
        return unsafeSort();
    }


    final Iter0IterableSetOperator<T> unsafeSort(final Comparator<? super T> comparator) {
        
        final SetTarget<T> target = getTargets().get(0);
         ...
         */
        final List<T> newTargetListObject = new ArrayList<T>(target.getObject());
        Collections.sort(newTargetListObject, comparator);
        final Set<T> newTargetObject = new LinkedHashSet<T>(newTargetListObject);
        

        return new Iter0IterableSetOperator<T>(
                getOperatorComponentType(),
                target.getSetComponentType(),
                newTargetObject);
        
    }


    public final Iter0IterableSetOperator<T> sort(final Comparator<? super T> comparator) {
        // Comparator can be null. This is to adhere to the Collections.sort(...) API.
        return unsafeSort(comparator);
    }


    final Iter0IterableSetOperator<T> unsafeEachFilter(
            final String expression, final List<Object> parameters) {
        if (areAllTargetsNull()) {
            throw new NullTargetException("eachFilter");
        }
        return unsafeEach().unsafeFilter(expression, parameters).uneachSet(); 
    }

    
    final Iter0IterableSetOperator<T> unsafeEachFilter(final Filter<T> filter) {
        if (areAllTargetsNull()) {
            throw new NullTargetException("eachFilter");
        }
        return unsafeEach().unsafeFilter(filter).uneachSet(); 
    }


    public final Iter0IterableSetOperator<T> eachFilter(
    ...
    public final Iter0IterableSetOperator<T> eachFilter(final Filter<T> filter) {
    ...
    final Iter0IterableListOperator<T> unsafeToList() {
        return new Iter0IterableListOperator<T>(getOperatorComponentType(), new ArrayList<T>(getTargetObjects().get(0)));
    }
    ...
    final <X> Iter0IterableSetOperator<X> unsafeEachEval(
            final Class<X> resultClass, final String expression, final List<Object> parameters) {
        return unsafeEach().unsafeEval(resultClass, expression, parameters).unsafeUneachSet();
    }

    
    final <X> Iter0IterableSetOperator<X> unsafeEachEval(
            final Class<X> resultClass, final Evaluator<T,X> evaluator) {
        return unsafeEach().unsafeEval(resultClass, evaluator).unsafeUneachSet();
    }

    
    public final Iter0IterableSetOperator<?> eachEval(
    ...
    public final <X> Iter0IterableSetOperator<X> eachEval(
    ...
    final <X> Iter0IterableSetOperator<X> unsafeEachCall(
            final Class<X> resultClass, final String methodName, final List<Object> arguments) {
        if (areAllTargetsNull()) {
            throw new NullTargetException("eachCall");
        }
        return unsafeEach().unsafeCall(resultClass, methodName, arguments).unsafeUneachSet();
    }
    

    public final Iter0IterableSetOperator<?> eachCall(
    ...
    public final <X> Iter0IterableSetOperator<X> eachCall(
    ...
    final <X> Iter0IterableSetOperator<X> unsafeEachTo(
            final Class<X> resultClass, final Type resultType, final List<Object> parameters) {
        return unsafeEach().unsafeTo(resultClass, resultType, parameters).unsafeUneachSet();
    }
    

    public final Iter0IterableSetOperator<?> eachTo(
    ...
    public final <X> Iter0IterableSetOperator<X> eachTo(
    ...
    public final <X> Set<X> get(Class<X> componentClass) {
        return unsafeAsSet(componentClass).getTargetObjects().get(0);
    }
